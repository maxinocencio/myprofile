...
class Sobremim(Toplevel):                           # Criando a segunda tela (Sobre mim)
    ...
    def __init__(self, original):                   # Método construtor
        ...
        self.widgetsdesc()                          # Chamando os widgets do frame desc     (Texto, Meu nome é João, tenho 17 anos. Gosto de músicas e jogos. Trabalho com programação em python)
        self.widgetsbotao()                         # Chamando os widgets do frame botao    (Botão, Voltar)

    def config(self):                               # Configurações da janela 2
    ...
    def frames(self):                               # Frames Janela 2
        ...
        self.desc = Frame(                          # Criando frame desc (descrição)
            self,
            bg = self.corprincipal                  # Determinando a cor de fundo do frame
        )

        self.desc.place(                            # Determinando onde o frame desc ficará
            relx = 0.1,
            rely = 0.25,
            relwidth = 0.75,
            relheight = 0.2
    ...
    def widgetstitulo(self):                                    # Widgets do frame titulo                 
        title = Label(self.titulo,                              # Mostrando que pertence ao frame titulo
            text='Sobre mim',                                   # Texto que será inserido no frame
    ...
    def widgetsdesc(self):                                      # Widgets do frame desc
        desc = Label(self.desc,

            text = 'Meu nome é João, tenho 17 anos.\nGosto de músicas e jogos. \n \nTrabalho com programação \nem python',                                            # Texto que será inserido no frame
            font=('Poppins', 11, 'bold'),                       # Fonte usada no texto
            bg = self.corprincipal,                             # Cor backgroud do texto
            fg = self.cor1,                                     # Cor do texto
            justify=LEFT,                                       # Justifica o texto na esquerda
        )
        desc.pack(side = LEFT)                                  # "Empacotar" o texto a esquerda
